...
// The next section deals with the euler explicit method
   ...
   while (time <= t) {
      ...
      for (int i=0; i<=x_spaces;i++){
         if (i==0){
            ...
            u_new_values[i] = u_old_values[i] - c*delta_t/(2*delta_x)*(u_old_values[i+1]-u_old_values[i-1])+pow(c,2)*pow(delta_t,2)/(2*pow(delta_x,2))*(u_old_values[i+1]-2*u_old_values[i]+u_old_values[i-1]);
         }
      }
   ...
   for (int j=0; j<=x_spaces;j++){
      if (j==0){
         cout << "X Lax Value Start" << "," << x_ee_values[j] << ",";
      }else if (j==x_spaces){
         cout << x_ee_values[j] << "," << "X Lax Value End" << ",";
      }else{
         cout << x_ee_values[j] << ",";
      }
   ...
   for (int k=0; k<=x_spaces;k++){
      if (k==0){
         cout << "Y Lax Value Start" << "," << u_values[k] << ",";
      }else if (k==x_spaces){
         cout << u_values[k] << "," << "Y Lax Value End" << ","<< endl;
      }else{
         cout << u_values[k] << ",";
      }
